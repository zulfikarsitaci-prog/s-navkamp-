...
# --- PERFORMANCE: Cache Süresi Artırıldı ---
...
def create_database():
    tables = [
        'CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT UNIQUE, password TEXT, role TEXT, last_seen TEXT, avatar_data TEXT, frame TEXT, name_style TEXT, post_style TEXT, font_style TEXT)',
    ...
    cols = ["avatar_data", "frame", "name_style", "post_style", "font_style"]
    for col in cols:
...
# --- CACHED GETTERS (HIZ İÇİN) ---
@st.cache_data(ttl=2) # Postlar çok hızlı değişebilir
...
def get_user_styles(u):
    try: 
        res = run_query("SELECT avatar_data, frame, name_style, post_style, font_style FROM users WHERE username = ?", (u,), fetch=True)
