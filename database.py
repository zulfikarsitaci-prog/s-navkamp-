...
# --- VERİTABANI BAĞLANTI (HATA GÖSTEREN MOD) ---
...
def run_query(query, params=(), fetch=False):
    ...
    # Postgres bağlantısı koptuysa tazele
    ...
    try:
        ...
        # BURASI ÖNEMLİ: Hatayı ekrana basıyoruz
...
def create_database():
    ...
    for t in tables:
        success = run_query(t)
        if not success: st.warning(f"Tablo oluşturulamadı: {t[:30]}...")
...
def add_user(u, p, r):
    try:
        ...
        st.error(f"Kullanıcı ekleme hatası: {e}")
        return False
...
def get_all_users():
    return run_query("SELECT username, role, last_seen FROM users", fetch=True) or []
...
def get_unread_messages(u):
    return run_query("SELECT id, sender, message FROM messages WHERE receiver = ? AND is_read = 0", (u,), fetch=True) or []

def get_my_messages(u):
    return run_query("SELECT id, sender, message, timestamp, is_read FROM messages WHERE receiver = ? ORDER BY id DESC", (u,), fetch=True) or []
