...
def get_searchable_users(my_u):
    all_u = get_all_users_list(my_u)
    friends = get_friends(my_u)
    return [u for u in all_u if u not in friends and u != my_u]
def send_friend_request(s, r):
    if run_query("SELECT * FROM relationships WHERE (user1=? AND user2=?) OR (user1=? AND user2=?)", (s, r, r, s), fetch=True): return False, "İstek var."
