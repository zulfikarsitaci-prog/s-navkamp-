...
# --- CSS: Çerçeveler ve Bildirim Rozeti ---
st.markdown("""
<style>
    ...
    /* Çerçeve Stilleri (Basit CSS halkalar) */
    ...
    .frame-cyber { border: 3px solid #ff00ff; border-style: dashed; border-radius: 50%; box-shadow: 0 0 5px #ff00ff; }
...
# --- YARDIMCI FONKSİYON: AVATAR HTML ---
def get_avatar_html(username, size=50):
    ...
    if frame_type:
        ...
        # Çerçeveyi bir div olarak ekliyoruz, resmin üzerine gelecek
        frame_html = f'<div class="frame-overlay {frame_class}" style="width:{size+10}px;height:{size+10}px;top:-5px;left:-5px;"></div>'
...
if not st.session_state['logged_in']:
    ...
    # --- SOL MENÜ ---
    with st.sidebar:
        # Avatarı ve Varsa Çerçeveyi Göster
        st.markdown(get_avatar_html(st.session_state['username'], size=80), unsafe_allow_html=True)
        st.title(st.session_state['username'])
        
        # Profil Resmi Yükleme (Artık Sıkıştırmalı)
        uploaded_avatar = st.file_uploader("Profil Resmi Güncelle", type=['png', 'jpg', 'jpeg'])
        if uploaded_avatar:
    ...
    # --- DİNAMİK MENÜ (Bildirim Sayılı) ---
    noti_count = database.get_unread_notification_count(st.session_state['username'])
    noti_text = f"🔔 Bildirimler ({noti_count})" if noti_count > 0 else "🔔 Bildirimler"
    
    menu_ops = ["📢 Kampüs Duvar", "💬 Mesaj", "🏆 Puan", "📚 Ders", "🎮 Oyun", "🛒 Mağaza", noti_text]
    if st.session_state['user_role'] == 'admin': menu_ops.append("⚙️ Admin")
    
    # Menü seçim mantığı (Bildirim yazısı değiştiği için kontrol gerek)
    current_selection = st.session_state['active_menu']
    if current_selection.startswith("🔔 Bildirimler") and current_selection != noti_text:
        current_selection = noti_text # Yazı değiştiyse yenisine geç
        
    ix = 0
    if current_selection in menu_ops: ix = menu_ops.index(current_selection)
            ...
            with st.container():
                ...
                if comments:
                    with st.expander(f"💬 Yorumlar ({len(comments)})"):
                        for c in comments: 
                            # Yorumlarda da avatar göster
                            c_ava_html = get_avatar_html(c[0], size=25)
                            st.markdown(f"""<div class='comment-sec' style="display:flex;align-items:center;">
                                            {c_ava_html} <div style="margin-left:5px;"><b>{c[0]}</b>: {c[1]}</div>
                                        </div>""", unsafe_allow_html=True)
                with st.popover("Yorum Yap"):
    ...
    # --- YENİ: MAĞAZA ---
        ...
        st.metric("Mevcut Puanın", my_score)
        st.write("Puanlarınla profilin için havalı çerçeveler al!")
        
        c1, c2, c3 = st.columns(3)
        frames = [
            {"name": "Altın", "cost": 5000, "css": "frame-gold", "desc": "Parlak altın bir halka."},
            {"name": "Neon", "cost": 15000, "css": "frame-neon", "desc": "Siberpunk mavi neon ışık."},
            {"name": "Siber", "cost": 30000, "css": "frame-cyber", "desc": "Kesik çizgili magenta çerçeve."}
        ]
        
        for i, col in enumerate([c1, c2, c3]):
            f = frames[i]
            with col:
                st.subheader(f"{f['name']} Çerçeve")
                # Önizleme
                st.markdown(f"""<div style="position:relative;width:60px;height:60px;margin:0 auto;">
                                <img src="https://via.placeholder.com/60" style="border-radius:50%;">
                                <div class="{f['css']}" style="position:absolute;top:-5px;left:-5px;width:70px;height:70px;"></div>
                            </div>""", unsafe_allow_html=True)
                st.write(f"**Fiyat:** {f['cost']} Puan")
                st.caption(f["desc"])
                if st.button(f"Satın Al ({f['name']})", key=f"buy_{i}"):
                    ok, msg = server.buy_item(st.session_state['username'], "frame", f["css"], f["cost"])
                    if ok: st.success(msg); time.sleep(1); st.rerun()
                    else: st.error(msg)
    
    # --- YENİ: BİLDİRİMLER ---
    elif sel.startswith("🔔 Bildirimler"):
        st.header("Bildirimler")
        notis = database.get_unread_notifications(st.session_state['username'])
        if not notis:
            st.info("Yeni bildirim yok.")
        else:
            for who, comment, post_summary in notis:
                st.warning(f"**{who}** senin gönderine yorum yaptı: _'{comment}'_ (Gönderi: {post_summary[:30]}...)")
            
            # Sayfa açıldığında okundu olarak işaretle
            database.mark_notifications_read(st.session_state['username'])
            # Menüdeki sayıyı güncellemek için bir kez daha rerun gerekebilir ama şimdilik kalsın.
        ...
        if target:
            for s, m, t in database.get_conversation(st.session_state['username'], target):
                ...
                st.markdown(f"""<div style='display:flex;{align};align-items:center;margin:5px;'>
                                {ava_html} <div style='padding:10px;border-radius:10px;margin:5px;color:white;background:inherit'>{m}</div>
                            </div>""", unsafe_allow_html=True)
