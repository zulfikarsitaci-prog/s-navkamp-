...
# --- 5. VERİ YÜKLEME (GÜÇLENDİRİLMİŞ) ---
def dosya_yukle(dosya_adi):
    if not os.path.exists(dosya_adi): 
        # Dosya yoksa sessiz kalma, kullanıcıya göster!
        st.error(f"🚨 HATA: '{dosya_adi}' dosyası bulunamadı! Lütfen GitHub'a yüklediğinden emin ol.")
        return {}
    try:
        with open(dosya_adi, "r", encoding="utf-8") as f:
            data = json.load(f)
            # TYT verisi için string key'leri integer'a çevir (Örn: "1" -> 1)
            if dosya_adi == TYT_JSON_ADI:
                return {int(k): v for k, v in data.items()}
            return data
    except json.JSONDecodeError as e:
        st.error(f"🚨 FORMAT HATASI: '{dosya_adi}' bozuk. İçinde virgül veya parantez hatası var.\nDetay: {e}")
        return {}
    except Exception as e:
        st.error(f"🚨 BİLİNMEYEN HATA: '{dosya_adi}' okunamadı. Detay: {e}")
        return {}
...
def pdf_sayfa_getir(yol, sayfa):
    if not os.path.exists(yol): 
        st.warning(f"⚠️ PDF Dosyası ({yol}) bulunamadı.")
        return
    try:
        doc = fitz.open(yol)
...
def get_hybrid_leaderboard(current_user, current_score):
    try:
        # 1. Google Sheets'ten veriyi çek
        ...
        if name_col and score_col:
            for _, row in df.iterrows():
                try:
                    ...
                    if puan > 0: # Sadece 0'dan büyükleri al
                        data.append({"name": str(row[name_col]), "score": puan})
                except: continue
        
        # 2. ŞU ANKİ KULLANICIYI LİSTEYE EKLE
        user_exists = False
        for p in data:
...
if st.session_state.ekran == 'giris':
    ...
    if st.session_state.secim_turu == "GAME":
        ...
        leaderboard_json = get_hybrid_leaderboard(st.session_state.ad_soyad, 0)
    ...
    # LIFE-SIM SAYFASI
        ...
        with col_ls_2:
            ...
            st.text_area("Analiz Raporu:", placeholder="Bu kararın sonuçları...")
            if st.button("✅ Analizi Tamamla ve Ödülü Al (+250 ₺)", use_container_width=True):
