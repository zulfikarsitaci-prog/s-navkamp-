    ...
    data = {
      "9. Sınıf": {
        "Mesleki Gelişim Atölyesi": [
          ...
          {"type": "text", "question": "4. E-Posta vs WhatsApp farkı?", "answer": "Resmiyet", "points": 10},
          {"type": "text", "question": "5. Staj maili konusu?", "answer": "Staj Başvurusu", "points": 10},
      ...
      "10. Sınıf": {
        "Ekonomi": [{"type": "text", "question": "Üretim faktörü?", "answer": "Emek", "points": 10}]
      },
      "11. Sınıf": {
        "Şirketler Muhasebesi": [{"type": "text", "question": "Anonim şirket kurucu sayısı?", "answer": "1", "points": 10}]
      }
    }
...
<style>
    ...
    /* POST KARTI */
    .post-card { background-color: #1e293b; border: 1px solid #334155; border-radius: 12px; padding: 10px; margin-bottom: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.3); }
    ...
    div[data-testid="column"] { padding: 0 2px !important; min-width: 0 !important; margin: 0 !important; flex: 1 1 0 !important; }
    ...
    .shop-preview { width: 35px; height: 35px; margin-bottom: 2px; display: flex; align-items: center; justify-content: center; border-radius: 50%; border: 1px solid #334155; }
    .shop-name { font-size: 0.65rem; color: #cbd5e1; font-weight: bold; margin-bottom: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 100%;}
    
    /* BUTON KUTUSU */
    .shop-btn-box { width: 100%; margin-top: 2px; }
    .shop-btn-box button {
...
def get_matrix_game_html(user): return "Oyun Yükleniyor..."
...
if not st.session_state['logged_in']:
    ...
    if sel == "Duvar":
        ...
        def render_shop_tab(items, type_key):
            ...
            for row in rows:
                ...
                for i, it in enumerate(row):
                    with cols[i]:
                        preview = f'<div class="shop-preview"><div class="{it.get("css", "")}" style="width:100%;height:100%;border-radius:50%;"></div></div>' if type_key == "frame" else f'<div style="font-size:0.7rem;margin-bottom:2px;">Aa</div>'
                        st.markdown(f'<div class="shop-item">{preview}<div class="shop-name">{it["n"]}</div><div class="shop-btn-box"></div></div>', unsafe_allow_html=True)
                        if st.button(f"AL ({it['c']//1000}K)", key=f"buy_{type_key}_{it['n']}"):
        ...
        with tabs[0]:
            items = [{"n":"Gold","c":50000,"t":"frame","v":"Gold","css":"frame-Gold"}, {"n":"Neon","c":150000,"t":"frame","v":"Neon","css":"frame-Neon"}, {"n":"Alev","c":300000,"t":"frame","v":"Fire","css":"frame-Fire"}, {"n":"Kral","c":2000000,"t":"frame","v":"King","css":"frame-King"}, {"n":"Buz","c":750000,"t":"frame","v":"Ice","css":"frame-Ice"}, {"n":"Doğa","c":250000,"t":"frame","v":"Nature","css":"frame-Nature"}, {"n":"Siber","c":600000,"t":"frame","v":"Cyber","css":"frame-Cyber"}, {"n":"Aşk","c":400000,"t":"frame","v":"Love","css":"frame-Love"}]
        ...
        with tabs[3]: 
            ...
            for row in rows:
                ...
                for i, g in enumerate(row):
                    with cols[i]:
                        st.markdown(f'<div class="shop-item"><div class="shop-name">{g["n"]}</div><div class="shop-btn-box"></div></div>', unsafe_allow_html=True)
                        if st.button(f"GÖNDER ({g['c']//1000}K)", key=f"g_{g['n']}"):
        ...
        u_list = [u[0] for u in all_u]
        tu = st.selectbox("Kullanıcı", u_list)
