...
def create_full_exams_json():
    data = {
      "9. Sınıf": {
        ...
        "Temel Muhasebe": [
          {"type": "calculation", "text": "KIDEM TAZMİNATI: 4 Yıl 5 Ay 18 Gün. Brüt: 30.000 TL.", "inputs": [{"label": "4 Yıllık", "correct": 120000}, {"label": "5 Aylık", "correct": 12500}, {"label": "18 Günlük", "correct": 18000}, {"label": "TOPLAM", "correct": 150500}], "points": 20},
          {"type": "text", "question": "Kasa hesabı kodu?", "answer": "100", "points": 10},
          {"type": "text", "question": "Bilançonun sol tarafı?", "answer": "Aktif", "points": 10}
        ]
      },
      "10. Sınıf": {
        "Ekonomi": [
            {"type": "text", "question": "Söke'de tarım?", "answer": "Pamuk", "points": 10},
            {"type": "text", "question": "Üretim faktörlerinden biri?", "answer": "Emek", "points": 10}
        ]
      },
      "11. Sınıf": {
        "Şirketler Muhasebesi": [
            {"type": "text", "question": "Anonim şirket en az kaç kişi?", "answer": "1", "points": 10}
        ]
      }
    }
...
<style>
    ...
    /* ESKİ GÜZEL MAĞAZA TASARIMI (Grid Düzeltildi) */
    ...
    .shop-item { 
        ...
        padding: 10px; 
        text-align: center; 
        height: 150px; /* Eski boyuta yakın */
        display: flex; 
    ...
    .shop-name { font-size: 0.75rem; color: #cbd5e1; font-weight: bold; margin-bottom: 2px; }
    
    /* BUTON KUTUSU - Streamlit butonlarını içine almak için */
    .shop-btn-box { width: 100%; margin-top: 5px; }
    /* Streamlit butonunu yeşil yap */
    .shop-btn-box button {
...
if not st.session_state['logged_in']:
    ...
    if sel == "📢 Kampüs Duvar":
        ...
        # --- ORTAK RENDER FONKSİYONU ---
        def render_shop_tab(items, type_key):
            # Mobilde 2, PC'de 4 kolon olması için chunking
            rows = [items[i:i+4] for i in range(0, len(items), 4)]
            for row in rows:
                cols = st.columns(4)
                for i, it in enumerate(row):
                    with cols[i]:
                        # CSS ile hizalanmış HTML
                        preview = ""
                        if type_key == "frame":
                            preview = f'<div class="shop-preview"><div class="{it["css"]}" style="width:100%;height:100%;border-radius:50%;"></div></div>'
                        elif type_key == "name":
                            preview = f'<div class="{it["css"]}" style="font-size:0.8rem;margin-top:20px;">İSİM</div>'
                        elif type_key == "font":
                            preview = f'<div class="{it["css"]}" style="font-size:1rem;margin-top:20px;">Aa</div>'
                        
                        st.markdown(f"""
                        <div class="shop-item">
                            {preview}
                            <div class="shop-name">{it['n']}</div>
                            <div class="shop-btn-box"></div>
                        </div>
                        """, unsafe_allow_html=True)
                        
                        # BUTON (HTML değil, Streamlit butonu)
            ...
            for row in rows:
                ...
                for i, (gn, gp) in enumerate(row):
                    with cols[i]:
                        st.markdown(f"""
                        <div class="shop-item">
                            <div style="font-size:1.5rem;margin-top:10px;">{gn.split()[-1]}</div>
                            <div class="shop-name">{gn}</div>
                            <div class="shop-btn-box"></div>
                        </div>""", unsafe_allow_html=True)
                        if st.button(f"GÖNDER ({gp//1000}K)", key=f"gift_{gn}"):
