...
# --- KRİTİK: VERİTABANI BAŞLATMA VE ONARMA ---
try:
    ...
    st.error(f"Veritabanı başlatılamadı. Lütfen sol menüden SIFIRLA butonuna basın. Hata: {e}")
...
if "t_user" in st.query_params and "t_amt" in st.query_params:
    try:
        ...
        if role:
            ...
            st.session_state['active_menu'] = "🎮 Oyun" # Oyun sayfasına geri dön
            if a > 0: 
...
if not st.session_state['logged_in']:
    ...
    with c2:
        ...
        with st.expander("Kayıt Ol"):
            with st.form("reg"):
                nu = st.text_input("Yeni Kullanıcı Adı"); np = st.text_input("Şifre", type="password")
                if st.form_submit_button("Kayıt"):
                    if database.add_user(nu, np, "student"): st.success("Başarılı! Giriş yap."); st.rerun()
                    else: st.error("Kullanıcı adı alınmış.")
    
    # --- KURTARMA BUTONU (SOL MENÜ) ---
    st.sidebar.warning("Giriş yapamıyor musun?")
    if st.sidebar.button("🔴 SİSTEMİ SIFIRLA (RESET)"):
        if os.path.exists("education_platform.db"):
            os.remove("education_platform.db")
            st.success("Sistem sıfırlandı! Sayfa yenileniyor...")
            time.sleep(2)
            st.rerun()
