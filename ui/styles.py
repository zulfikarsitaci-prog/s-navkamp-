...
<style>
    ...
    /* --- STICKY HEADER (SABİT ÜST MENÜ) --- */
    .top-bar { 
        position: sticky;
        top: 0;
        z-index: 999;
        background: rgba(30, 41, 59, 0.95); /* Arkası hafif görünen koyu zemin */
        padding: 12px 15px; 
        ...
        margin: -1rem -1rem 1rem -1rem; /* Kenar boşluklarını sıfırla */
        box-shadow: 0 5px 20px rgba(0,0,0,0.5);
    }
    
    /* --- GLASSMORPHISM POST KARTI (BUZLU CAM EFEKTİ) --- */
    .post-card {
        background: rgba(30, 41, 59, 0.65); /* Şeffaflık artırıldı */
        backdrop-filter: blur(12px);         /* Bulanıklık efekti */
        -webkit-backdrop-filter: blur(12px);
        border: 1px solid rgba(255, 255, 255, 0.08); /* İnce parlak kenarlık */
        border-radius: 16px;
        padding: 15px;
        margin-bottom: 15px;
...
def get_user_display_html(username, size=40):
    ...
    # Boyut ayarı dinamik hale getirildi (avatar-img class'ına inline stil eklendi)
