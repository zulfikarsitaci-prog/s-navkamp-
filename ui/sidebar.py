...
def render_xp_bar(current_score):
    ...
    for i in range(len(ranks)-1):
        if current_score >= ranks[i][0]: current_rank = ranks[i]; next_rank = ranks[i+1]
        else: break
    if current_score >= ranks[-1][0]: progress = 100; label = "MAX SEVİYE"
    else:
        range_diff = next_rank[0] - current_rank[0]; score_diff = current_score - current_rank[0]
                ...
                with st.form("sidebar_story_form", clear_on_submit=True):
                    ...
                    if st.form_submit_button("Paylaş"):
                        if st_img:
                            ...
                            st.toast("Hikaye paylaşıldı!", icon="✅"); time.sleep(1); st.rerun()
